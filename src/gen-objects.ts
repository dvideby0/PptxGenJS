	...
	if (props.objects && Array.isArray(props.objects) && props.objects.length > 0) {
		props.objects.forEach((object, idx) => {
			...
			if (MASTER_OBJECTS[key] && key === 'chart') addChartDefinition(tgt, object[key].type, object[key].data, object[key].opts)
					...
					const textOptions = Array.isArray(object[key]) ? object["options"] : object[key].options;
...
export function addTableDefinitionToMaster(
	target: SlideLayout,
	tableRows: TableRow[],
	options: TableProps
  ): void {
	...
	opt.objectName = opt.objectName
	  ...
	  : `Table ${
		  target._slideObjects.filter((obj) => obj._type === SLIDE_OBJECT_TYPES.table).length
		}`;
	...
	// STEP 1: Reality-check
	{
	  ...
	  if (!Array.isArray(tableRows) || tableRows.length === 0) {
		throw new Error(
		  "addTableDefinitionToMaster: Array of rows expected! EX: 'slide.addTable( [rows], {options} );'"
		);
	  }
	  ...
	  if (!Array.isArray(tableRows[0])) {
		throw new Error(
		  "addTableDefinitionToMaster: 'rows' should be an array of cells! EX: 'slide.addTable( [ ['A'], ['B'] ] );'"
		);
	  }
	}
	...
	tableRows.forEach((row) => {
	  ...
	  if (Array.isArray(row)) {
		row.forEach((cell: any) => {
		  ...
		  [0, 1, 2, 3].forEach((idx) => {
			if (!newCell.options.border[idx]) newCell.options.border[idx] = { type: 'none' };
			newCell.options.border[idx] = {
			  type: newCell.options.border[idx].type || DEF_CELL_BORDER.type,
			  color: newCell.options.border[idx].color || DEF_CELL_BORDER.color,
			  pt:
		...
		console.warn(
		  'addTableDefinitionToMaster: Each row should be an array of cells. Skipping invalid row:',
		  row
		);
	  }
	...
	if (opt.h) opt.h = getSmartParseNumber(opt.h, 'Y', target._presLayout);
	opt.fontSize = opt.fontSize || DEF_FONT_SIZE;
	opt.margin =
	...
	const presLayout = target._presLayout || { width: 10, height: 7.5 }; // Default to standard size if not available
